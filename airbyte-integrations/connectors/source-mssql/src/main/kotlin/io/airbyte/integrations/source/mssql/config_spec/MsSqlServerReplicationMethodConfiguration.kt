...
sealed interface MsSqlServerReplicationMethodConfiguration
...
class CDC : MsSqlServerReplicationMethodConfiguration {
    ...
    @JsonPropertyDescription(
        "The amount of time the connector will wait when it launches to determine if there is new data to sync or not. " +
            ...
            "Read about <a href=\"https://docs.airbyte.com/integrations/sources/mysql/#change-data-capture-cdc\">initial waiting time</a>.",
    )
...
data object Standard : MsSqlServerReplicationMethodConfiguration

class MicronautPropertiesFriendlyMsSqlServerReplicationMethodConfiguration {
    val method: String = "CDC"
    @JsonValue
    fun asReplicationMethod(): MsSqlServerReplicationMethodConfiguration =
        when (method) {
            "CDC" -> CDC()
            "STANDARD" -> Standard
